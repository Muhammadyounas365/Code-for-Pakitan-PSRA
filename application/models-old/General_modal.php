...
class General_modal extends MY_Model{
    ...
    public function districts($id = 0, $list = true){
      ...
      $data = '';
          if (count($result) > 0 ) {
              $data = "<option>Select District</option>";
              foreach ($result as $item) {
    ...
    public function tehsils($id = 0, $list = true){
      ...
      if($id != 0){
      $query = "SELECT `tehsilId`, `tehsilTitle`, `district_id` FROM `tehsils` WHERE `district_id` = '".$id."';";
      }
      $result = $this->db->query($query)->result();
      if($list == true){
        return $result;
          ...
          $data = '';
          if (count($result) > 0 ) {
              $data = "<option>Select Tehsil</option>";
              foreach ($result as $item) {
    ...
    public function ucs($id = 0, $list = true){
      ...
      if($id != 0){
      $query = "SELECT `ucId`, `ucTitle`, `tehsil_id` FROM `uc` WHERE `tehsil_id` = '".$id."';";
      }
      $result = $this->db->query($query)->result();
      if($list == true){
        return $result;
